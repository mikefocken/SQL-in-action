...
select Distinct host_name from final_airbnb where host_id="63613" order by host_name;   
...
select Distinct neighbourhood from final_airbnb order by neighbourhood;
...
select  max(price),min(price)from final_airbnb;
...
select* from final_airbnb where number_of_reviews is null or number_of_reviews =0;
...
select room_type,number_of_reviews from final_airbnb where room_type = 'private room' and number_of_reviews = max order by id;  
